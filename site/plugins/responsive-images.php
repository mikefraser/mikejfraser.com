...
//     $srcset  = $tag->attr('srcset');
    ...
    // srcset builder
    // I don't want to allow custom srcset within kirbytext
...
// Extend $file object for template use
// Code copied from above, but with no link wrapping / figure element - do that yourself in the template
// - just returns img tag

file::$methods['respImage'] = function($file, $args = array() ) {
...
*/
	...
	// try to get the title from the image object and use it as alt text
	if($file) {
		if(empty($config['alt']) and $file->alt() != '') {
			$config['alt'] = $file->alt();
		}
		if(empty($config['title']) and $file->title() != '') {
			$config['title'] = $file->title();
		}
	}
	
	// srcset builder
	if($file && empty($config['srcset'])) {
		$config['srcset'] = kirby_get_srcset($file, array( 'minsrc' => $config['minsrc'], 'maxsrc' => $config['maxsrc']) );
	}
	
	// sizes builder
	if($file && empty($config['sizes'])) {
// 		$classes = ( ! empty( $tag->attr('imgclass'))) ? explode( ' ', $tag->attr('imgclass')) : '';
		$config['sizes'] = kirby_get_sizes($file);
	}
	// allows src attribute to be overwritten
	$defaultsource = kirby()->option('responsiveimages.defaultsource');
	if ( isset($sources[$defaultsource])) {
		$config['url'] = thumb($file, $sources[$defaultsource])->url();
	}
	
	return html::img($config['url'], array(
// 			'width'  => $tag->attr('width'),
// 			'height' => $tag->attr('height'),
		'class'  => $config['class'],
		'title'  => $config['title'],
 ...
 *  @return  string
 */
function kirby_get_srcset( $file, $range = false, $sources_provided = false ) {
//     $srcset = $file->url() .' '. $file->width() .'w';
	$srcset = '';
	if ( $sources_provided ) {
		$sources_arr = $sources_provided;
	} else {
		$sources_arr = kirby_get_sources_array( $file );
	}
	
	// Enable customisation of srcset images output
	if ( $range === false ) {
		
		$defaultsize = kirby()->option('responsiveimages.defaultsource');
		if ( isset($defaultsize)  && !$sources_provided ) {
			$range['minsrc'] = $defaultsize;
		} else {
			$range['minsrc'] = false;
		}
		
		$maximumsize = kirby()->option('responsiveimages.inlinemax');
		if ( isset($maximumsize)  && !$sources_provided ) {
			$range['maxsrc'] = $maximumsize;
		} else {
			$range['maxsrc'] = false;
		}
	}
	
	$clone_source_arr = $sources_arr;
	
	// Strip out sizes that're too small
	if ( $range['minsrc'] !== false ) {
		foreach ($sources_arr as $key => $source) {
			if ( $key === $range['minsrc'] ) { break; }
			unset($clone_source_arr[$key]);
		}
	}
	// Strip out sizes that're too large
	if ( $range['maxsrc'] !== false ) {
		$sources_arr = array_reverse($sources_arr);
		foreach ($sources_arr as $key => $source) {
			if ( $key === $range['maxsrc'] ) { break; }
			unset($clone_source_arr[$key]);
		}
	}
	
	foreach ($clone_source_arr as $source) {
